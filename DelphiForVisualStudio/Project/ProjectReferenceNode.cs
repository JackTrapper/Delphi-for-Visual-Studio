...
namespace Microsoft.VisualStudio.Package
{
	...
	public class ProjectReferenceNode : TEntryPointFileNode
	{
		#region fieds
		/// <summary>
        ...
        internal string ReferencedProjectOutputPath
        {
            get
            {
                ...
                EnvDTE.Property assemblyNameProperty = ReferencedProjectObject.Properties.Item("OutputFileName");
		...
		/// </summary>
		public ProjectReferenceNode(ProjectNode root, ProjectElement e):base(root,e)
		{
			...
			// Do not die if the project setttings cannot be read.
			try
			{
		...
		/// </summary>
		public ProjectReferenceNode(ProjectNode root, string referencedProjectName, string projectPath, string projectReference):base(root)
		{
    ...
    protected override bool CanAddReference(out CannotAddReferenceErrorMessage errorHandler)
		...
		{
			string message = String.Format(CultureInfo.CurrentCulture, SR.GetString(SR.ProjectContainsCircularReferences), this.referencedProjectName);
		...
		private bool IsReferenceInCycle(Guid projectGuid)
		{
			...
			if (provider != null)
			{
				...
				foreach (TEntryPointFileNode refNode in referenceContainer.EnumReferences())
				{
