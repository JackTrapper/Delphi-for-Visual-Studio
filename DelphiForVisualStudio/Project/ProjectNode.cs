...
namespace Microsoft.VisualStudio.Package
{
  ...
  public abstract partial class ProjectNode : HierarchyNode,
    ...
    // BaseCodeChange: This was modifed to remove support for references as
    // Delphi Win32 does not support it.
    ...
    IVsProjectSpecialFiles
  {
    ...
    /// <summary>
    /// Control if file nodes can have child (sub items).
    ...
    /// <devdoc>The icons used in the hierarchy view.</devdoc>
    ...
    /// <summary>
    /// The folder where the project is craeted.
    /// </summary>
    ...
    /// <summary>
    /// Defines the project output sub folder. Not the full path. For example dir1\dir2\bin
    /// </summary>
    ...
    /// <summary>
    /// This property should be set if source control is to be disabled.
    /// </summary>
    ...
    public override int SetEditLabel(string label)
    {
      ...
      {
        throw new InvalidOperationException(String.Format(CultureInfo.CurrentCulture, SR.GetString(SR.PathTooLong), label));
      }
    ...
    public virtual VSADDRESULT RunWizard(HierarchyNode parentNode, string itemName, string wizardToRun, IntPtr dlgOwner)
    {
      ...
      {
        string errorMessage = String.Format(CultureInfo.CurrentCulture, SR.GetString(SR.PathTooLong), itemName);
      ...
      object[] vContextParams = new object[7];
      vContextParams[0] = EnvDTE.Constants.vsWizardAddItem;
      vContextParams[1] = this.Caption;
      ...
      if (automationObject is EnvDTE.Project)
      {
        ...
        vContextParams[2] = project.ProjectItems;
      }
      else
      {
        // This would normally be a folder unless it is an item with subitems
        ProjectItem item = (ProjectItem)automationObject;
        vContextParams[2] = item.ProjectItems;
      }

      vContextParams[3] = this.ProjectFolder;

      vContextParams[4] = itemName;
      ...
      vContextParams[5] = installDir;

      vContextParams[6] = true;

      EnvDTE.IVsExtensibility ivsExtensibility = (EnvDTE.IVsExtensibility)this.GetService(typeof(EnvDTE.IVsExtensibility));
      Debug.Assert(ivsExtensibility != null, "Failed to get EnvDTE.IVsExtensibility service");
      if (ivsExtensibility == null)
      {
        return VSADDRESULT.ADDRESULT_Failure;
      }

      EnvDTE.wizardResult wizResult = ivsExtensibility.RunWizardFile(wizardToRun, (int)dlgOwner, ref vContextParams);
      return (VSADDRESULT)wizResult;
    ...
    public virtual int AddProjectReference()
    {
      ...
      tabInit[1].guidTab = NativeMethods.GUID_BrowseFilePage;
    ...
    public virtual void PrepareBuild(string config, bool fCleanBuild)
    {
      ...
      if (fCleanBuild && this.buildProject.Targets.Exists(MsBuildTarget.Clean))
      {
    ...
    {
      if (String.Compare(Path.GetExtension(strFileName), ".ResX", StringComparison.OrdinalIgnoreCase) == 0)
        return true;
      return false;
    }
    ...
    public virtual HierarchyNode CreateFolderNodes(string strPath)
    {
      ...
      {
        throw new ArgumentNullException("starPath");
      }

      if (Path.IsPathRooted(strPath))
      {
        // Ensure we are using a relative path
        if (String.Compare(this.ProjectFolder, 0, strPath, 0, this.ProjectFolder.Length, StringComparison.OrdinalIgnoreCase) != 0)
          throw new ArgumentException();

        strPath = strPath.Substring(this.ProjectFolder.Length);
      }
      ...
      for (int i = 0; i < parts.Length; i++)
      {
        ...
        {
          strPath += parts[i] + "\\";
          curParent = VerifySubFolderExists(strPath, curParent);
        }
      }
      return curParent;
    }
    ...
    /// <param name="item">Item corresponding to the folder</param>
    /// <returns>A FolderNode that can then be added to the hierarchy</returns>
    protected internal virtual FolderNode CreateFolderNode(string path, ProjectElement item)
    {
      FolderNode folderNode = new FolderNode(this, path, item);
      return folderNode;
    }
    ...
    protected virtual void RenameProjectFile(string newFile)
    {
        ...
        && canContinue != 0)
      {
        ...
        try
        {
          ...
          // Actual file rename
          // BaseCodeChange: (1) This code would save the file and delete the 
          // old one and this would cause souce code controls to go out of sync
          //this.SaveMSBuildProjectFileAs(newFile);
          // (1) new Lines added...
          File.Move(oldFile, newFile);
          this.buildProject.FullFileName = newFile;
          this.filename = newFile;
          // BaseCodeChange: (2) the lines added support source code control
          // if is part of Scc and it should be checked in already so 
          // mark as readonly
          if (isRegisteredWithScc)
            File.SetAttributes(newFile, FileAttributes.ReadOnly | FileAttributes.Archive);
    ...
    protected virtual MSBuildResult InvokeMsBuild(string target)
    {
      ...
      // TODO: If the project has passed all security checks then do the actual build.
      try
      {
        ...
        if (this.buildProject != null)
        {
          this.buildProject.GlobalProperties.SetProperty(ProjectFileConstants.BuildingInsideVisualStudio, true.ToString());
          result = this.buildProject.Build(target) ? MSBuildResult.Sucessful : MSBuildResult.Failed;
        }
      }
      finally
      {
    ...
    protected virtual void InitializeProjectProperties()
    {
      ...
      // BaseCodeChange: this code add to the project file's XML <Name>
      // under <PropertyGroup> and would cause problems for socode control systems
    ...
    protected virtual int SaveAs(string newFileName)
    {
      ...
      {
        errorMessage = String.Format(CultureInfo.CurrentCulture, SR.GetString(SR.PathTooLong), newFileName);
      }
      else
      {
        ...
        if (errorMessage.Length == 0)
        {
          ...
          else
          {
            ...
            {
              errorMessage = String.Format(CultureInfo.CurrentCulture, SR.GetString(SR.SaveOfProjectFileOutsideCurrentDirectory), this.ProjectFolder);
            }
          }
        }
      }
    ...
    protected virtual void SaveMSBuildProjectFileAs(string newFileName)
    {
      ...
      // BaseCodeChange: (1) this code add to the project file's XML <Name>
      // under <PropertyGroup> and would cause problems for source code control systems
      // when the project was renamed.
    ...
    protected virtual int CanOverwriteExistingItem(string originalFileName, string computedNewFileName)
    {
      ...
      if (isOpen)
      {
        message = String.Format(CultureInfo.CurrentCulture, SR.GetString(SR.CannotAddFileThatIsOpenInEditor), Path.GetFileName(computedNewFileName));
      ...
      message = SR.GetString(SR.FileAlreadyInProject);
    ...
    /// <summary>
    /// Handle owerwriting of an existing item in the hierarchy from an an existing one. 
    /// NOTE:The file has already been copied.
    /// </summary>
    /// <param name="existingNode">The node that exists.</param>
    protected virtual void OverwriteExistingItemFrom(HierarchyNode existingNode, string aOriginalFile)  // BaseCodeChange: We need to know where the new file came from
    {
    ...
    protected virtual void AddNewFileNodeToHierarchyFrom(HierarchyNode parentNode, string fileName, string aOriginalFile)// BaseCodeChange: We need to know where the new file came from
    {
      HierarchyNode child;
      ...
      if (this.canFileNodesHaveChilds && (parentNode is FileNode || parentNode is DependentFileNode))
      {
        ...
        if (string.Compare(child.GetRelationalName(), parentNode.GetRelationalName(), true, CultureInfo.InvariantCulture) == 0)
        {
    ...
    /// </summary>
    /// <param name="outputgroupCanonicalName">Canonical name of the output group</param>
    /// <returns>Display name</returns>
    protected internal virtual string GetOutputGroupDisplayName(string outputgroupCanonicalName)
    {
      string result = SR.GetString(String.Format(CultureInfo.InvariantCulture, "Output{0}", outputgroupCanonicalName), CultureInfo.CurrentCulture);
      if (String.IsNullOrEmpty(result))
        result = outputgroupCanonicalName;
      return result;
    }
    ...
    protected internal virtual string GetOutputGroupDescription(string outputgroupCanonicalName)
    {
      string result = SR.GetString(String.Format(CultureInfo.InvariantCulture, "Output{0}Description", outputgroupCanonicalName, CultureInfo.CurrentCulture));
      if (String.IsNullOrEmpty(result))
        result = outputgroupCanonicalName;
      return result;
    }
    ...
    protected internal virtual void SetConfiguration(string config)
    {
      ...
      if (this.BuildInProgress)
      {
        ...
        bool configsAreEqual = String.Compare(currentConfigName, config, StringComparison.InvariantCultureIgnoreCase) == 0;
    ...
    /// <summary>
    /// Registers a project with the Scc manager.
    /// </summary>
    protected void RegisterSccProject()
    {
      ...
      if (sccManager != null)
      {
        NativeMethods.ThrowOnFailure(sccManager.RegisterSccProject(this, this.sccProjectName, this.sccAuxPath, this.sccLocalPath, this.sccProvider));
    ...
    protected void UnRegisterProject()
    {
      ...
      if (sccManager != null)
      {
        NativeMethods.ThrowOnFailure(sccManager.UnregisterSccProject(this));
    ...
    protected internal void LoadXmlFragment(IPersistXMLFragment persistXMLFragment, string configName)
    {
      if (xmlFragments == null)
      {
        ...
        fragments = String.Format("<root>{0}</root>", fragments);
    ...
    public virtual int AddItemWithSpecific(uint itemIdLoc, VSADDITEMOPERATION op, string itemName, uint filesToOpen, string[] files, IntPtr dlgOwner, uint editorFlags, ref Guid editorType, string physicalView, ref Guid logicalView, VSADDRESULT[] result)
    {
      ...
      for (int index = 0; index < filesToAdd.Count; index++)
      {
        ...
        if (NativeMethods.IsSamePath(file, newFileName) == false)
        {
          ...
          {
            string message = String.Format(CultureInfo.CurrentCulture, SR.GetString(SR.FileAlreadyExists), newFileName);
            string title = string.Empty;
        ...
        if (overwrite)
        {
          this.OverwriteExistingItemFrom(child, file);
        }
        else
        {
          //Add new filenode/dependentfilenode
          this.AddNewFileNodeToHierarchyFrom(n, newFileName, file);
        }
    ...
    public int BuildTarget(string targetName, out bool success)
    {
      ...
      if (result == MSBuildResult.Sucessful)
      {
        success = true;
      }
    ...
    private MSBuild.BuildProperty GetMsBuildProperty(string propertyName, bool resetCache)
    {
      ...
      if (this.currentConfig == null)
        throw new ApplicationException(String.Format(CultureInfo.CurrentCulture, SR.GetString(SR.FailedToRetrieveProperties), propertyName));
    ...
    private bool GetBoolAttr(MSBuild.BuildPropertyGroup properties, string name)
    {
      ...
      return (s != null && s.ToLower(CultureInfo.InvariantCulture).Trim() == "true");
    }
