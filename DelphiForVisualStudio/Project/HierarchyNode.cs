...
namespace Microsoft.VisualStudio.Package
{
  ...
  //, IVsBuildStatusCallback 
  {
    ...
    protected List<IVsHierarchy> ItemsDraggedOrCutOrCopied
    {
      get
      {
    ...
    protected HierarchyNode(ProjectNode root, ProjectElement e)
    {
    ...
    /// <summary>
    /// Set a property for a gived property id
    /// </summary>
    /// <param name="propid">the property id for the property to be set</param>
    ...
    public virtual void Remove(bool removeFromStorage)
    {
      ...
      // Notify listeners that the file is going to be removed.
      OnItemDeleted();
      ...
      {
        child.Remove(removeFromStorage);  // BaseCodeChange: This is a clear bug if removeFromStorage this node then delete child nodes 
      }
      ...
      VSREMOVEFILEFLAGS[] flags = this.GetRemoveFileFlags(filesToBeDeleted);
      Debug.Assert(flags != null, "At least an empty array should be returned for the GetRemoveFileFlags");

      this.ProjectMgr.Tracker.OnItemRemoved(documentToRemove, flags[0]);
    ...
    {
      return this.Caption.Substring(this.Caption.IndexOf(this.NameRelationSeparator));
    }
    ...
    protected void CloseDocumentWindow(HierarchyNode node)
    {
      ...
      if (NativeMethods.Succeeded(pRdt.GetRunningDocumentsEnum(out pEnumRdt)))
      {
    ...
    /// </summary>
    protected internal virtual int SetEditLabel(string label, string strRelPath)
    {
    ...
    protected virtual int AddNewFolder()
    {
      ...
      try
      {
        ...
        ErrorHandler.ThrowOnFailure(project.GenerateUniqueItemName(this.hierarchyId, String.Empty, String.Empty, out relFolder));

        // create the project part of it, the project file
        HierarchyNode child = this.ProjectMgr.CreateFolderNodes(Path.Combine(this.virtualNodeName, relFolder));
    ...
    protected virtual int ShowContextMenu(int menuID, Guid groupGuid, POINTS points)
    {
      IVsUIShell shell = this.projectMgr.Site.GetService(typeof(SVsUIShell)) as IVsUIShell;
      ...
      pnts[0].x = points.x;
      pnts[0].y = points.y;
      return shell.ShowContextMenu(0, ref groupGuid, menuID, pnts, (Microsoft.VisualStudio.OLE.Interop.IOleCommandTarget)this);
    }
    ...
    /// </summary>
    /// <param name="guidCmdGroup">Unique identifier of the command group</param>
    ...
    protected virtual int ExecCommandOnNode(Guid guidCmdGroup, uint cmd, uint nCmdexecopt, IntPtr pvaIn, IntPtr pvaOut)
    {
      ...
      else if (guidCmdGroup == VsMenus.guidVsUIHierarchyWindowCmds)
      {
        switch (cmd)
        {
      ...
      else if (guidCmdGroup == VsMenus.guidStandardCommandSet97)
      {
    ...
    /// <returns>If the method succeeds, it returns S_OK. If it fails, it returns an error code.</returns>
    protected virtual int ExecCommandThatDependsOnSelectedNodes(Guid guidCmdGroup, uint cmdId, uint cmdExecOpt, IntPtr vaIn, IntPtr vaOut, CommandOrigin commandOrigin, IList<HierarchyNode> selectedNodes, out bool handled)
    {
      handled = false;
    ...
    /// <param name="handled">An out parameter specifying that the command was handled.</param>
    /// <returns>If the method succeeds, it returns S_OK. If it fails, it returns an error code.</returns>
    protected virtual int ExecCommandIndependentOfSelection(Guid guidCmdGroup, uint cmdId, uint cmdExecOpt, IntPtr vaIn, IntPtr vaOut, CommandOrigin commandOrigin, out bool handled)
    {
      handled = false;
    ...
    /// <param name="commandOrigin">The origin of the command. From IOleCommandTarget or hierarchy.</param>
    /// <returns>If the method succeeds, it returns S_OK. If it fails, it returns an error code.</returns>
    protected virtual int InternalExecCommand(Guid guidCmdGroup, uint cmdId, uint cmdExecOpt, IntPtr vaIn, IntPtr vaOut, CommandOrigin commandOrigin)
    {
      CCITracing.TraceCall(guidCmdGroup.ToString() + "," + cmdId.ToString());
      if (this.projectMgr == null || this.projectMgr.IsClosed)
      {
        return (int)OleConstants.OLECMDERR_E_NOTSUPPORTED;
      }
      ...
      bool handled = false;
      int returnValue = this.ExecCommandIndependentOfSelection(guidCmdGroup, cmdId, cmdExecOpt, vaIn, vaOut, commandOrigin, out handled);
      if (handled)
      {
        return returnValue;
      }
      ...
      returnValue = this.ExecCommandThatDependsOnSelectedNodes(guidCmdGroup, cmdId, cmdExecOpt, vaIn, vaOut, commandOrigin, selectedNodes, out handled);
      if (handled)
      {
        return returnValue;
      }
      ...
      foreach (HierarchyNode node in selectedNodes)
      {
        try
        {
          returnValue = node.ExecCommandOnNode(guidCmdGroup, cmdId, cmdExecOpt, vaIn, vaOut);
        }
        catch (COMException e)
        {
          Trace.WriteLine("Exception : " + e.Message);
    ...
    protected virtual QueryStatusResult QueryStatusCommandFromOleCommandTarget(Guid guidCmdGroup, uint cmd, out bool handled)
    {
      handled = false;
    ...
    /// <param name="handled">Specifies whether the menu was handled.</param>
    /// <returns>A QueryStatusResult describing the status of the menu.</returns>
    protected virtual QueryStatusResult DisableCommandOnNodesThatDoNotSupportMultiSelection(Guid guidCmdGroup, uint cmd, IList<HierarchyNode> selectedNodes, out bool handled)
    {
      handled = false;
    ...
    /// <returns>If the method succeeds, it returns S_OK. If it fails, it returns an error code.</returns>
    protected virtual int QueryStatusOnNode(Guid guidCmdGroup, uint cmd, IntPtr pCmdText, ref QueryStatusResult result)
    {
    ...
    /// <param name="cmd">The command to be executed.</param>
    /// <returns>A QueryStatusResult describing the status of the menu.</returns>
    protected virtual bool DisableCmdInCurrentMode(Guid guidCmdGroup, uint cmd)
    {
      if (this.ProjectMgr == null || this.ProjectMgr.IsClosed)
      {
        return false;
      ...
      if (guidCmdGroup == VsMenus.guidStandardCommandSet97 || guidCmdGroup == VsMenus.guidStandardCommandSet2K)
      {
        ...
        {
          if (guidCmdGroup == VsMenus.guidStandardCommandSet97 && (VsCommands)cmd == VsCommands.Paste)
          {
            return true;
          }
        }
    ...
    /// </summary>
    /// <param name="guidCmdGroup">A unique identifier of the command group.</param>
    ...
    /// <returns>If the method succeeds, it returns S_OK. If it fails, it returns an error code.</returns>
    protected virtual int QueryStatusSelection(Guid guidCmdGroup, uint cCmds, OLECMD[] prgCmds, IntPtr pCmdText, CommandOrigin commandOrigin)
    {
      ...
      if (this.DisableCmdInCurrentMode(guidCmdGroup, cmd))
      {
        queryResult = QueryStatusResult.SUPPORTED | QueryStatusResult.INVISIBLE;
      }
      else
      {
        ...
        if (commandOrigin == CommandOrigin.OleCommandTarget)
        {
          queryResult = this.QueryStatusCommandFromOleCommandTarget(guidCmdGroup, cmd, out handled);
        }
        ...
        if (!handled)
        {
          ...
          if (selectedNodes != null && selectedNodes.Count > 1)
          {
            queryResult = this.DisableCommandOnNodesThatDoNotSupportMultiSelection(guidCmdGroup, cmd, selectedNodes, out handled);
          }

          // Now go and do the job on the nodes.
          if (!handled)
          {
            queryResult = this.QueryStatusSelectionOnNodes(selectedNodes, guidCmdGroup, cmd, pCmdText);
          }
    ...
    protected virtual QueryStatusResult QueryStatusSelectionOnNodes(IList<HierarchyNode> selectedNodes, Guid guidCmdGroup, uint cmd, IntPtr pCmdText)
    {
      ...
      foreach (HierarchyNode node in selectedNodes)
      {
        result = node.QueryStatusOnNode(guidCmdGroup, cmd, pCmdText, ref tempQueryResult);
        if (result < 0)
        {
          break;
        }
    ...
    protected internal virtual void GetSccFiles(IList<string> files, IList<tagVsSccFilesFlags> flags)
    {
      ...
      flags.Add(tagVsSccFilesFlags.SFF_NoFlags);
    }
    ...
    protected internal virtual void ReloadItem(uint reservered)
    {
    ...
    /// <summary>
    /// Causes the Ui hierarchy to be redrawn.
    /// </summary>
    ...
    public virtual int Close()
    {
      ...
      // If any of the children fail return the last failure.
      int returnValue = VSConstants.S_OK;
      int childClosedValue;

      // Walk tree closing any open docs that we own.
      for (HierarchyNode n = this.firstChild; n != null; n = n.nextSibling)
      {
        childClosedValue = n.Close();
        if (childClosedValue != VSConstants.S_OK && childClosedValue != VSConstants.S_FALSE)
        {
          returnValue = childClosedValue;
        }
      }
      return returnValue;
    }
    ...
    /// <param name="silentSaveAsName">Flags whose values are taken from the VSSAVEFLAGS enumeration.</param>
    ...
    public virtual int SaveItem(VSSAVEFLAGS saveFlag, string silentSaveAsName, uint itemid, IntPtr docData, out int cancelled)
    {
      ...
      {
        string errorMessage = string.Format(CultureInfo.CurrentCulture, SR.GetString(SR.CanNotSaveFileNotOpeneInEditor), node.Url);
      ...
      try
      {
        ...
        if ((VSSAVEFLAGS.VSSAVE_SilentSave & saveFlag) != 0)
        {
        ...
        else
        {
          ...
          // derived subclass implementation of SaveItemAs.

          // TODO: When dependent items and linked items are supported then lookup the implementation in \vsproject\vsproject\vsfilesaveasmgr.cpp
          //		 Due to the complexity of the problem a new SaveAsFileManager object would be necessary to handle all cases.
          if ((((saveFlag & VSSAVEFLAGS.VSSAVE_SaveAs) != 0) || (saveFlag & VSSAVEFLAGS.VSSAVE_SilentSave) != 0) && (cancelled == 0) && !String.IsNullOrEmpty(docNew) && !NativeMethods.IsSamePath(this.GetMkDocument(), docNew))
          {
            ...
            if ((returnCode == (int)OleConstants.OLECMDERR_E_CANCELED || returnCode == VSConstants.E_ABORT))
            {
              ...
              if (ff != null)
              {
                returnCode = shell.SaveDocDataToFile(VSSAVEFLAGS.VSSAVE_SilentSave, ff, node.GetMkDocument(), out docNew, out cancelled);
              }
            }
      ...
      catch (COMException e)
      {
        ...
        if (ff != null)
        {
          shell.SaveDocDataToFile(VSSAVEFLAGS.VSSAVE_SilentSave, ff, node.GetMkDocument(), out docNew, out cancelled);
        }
      }
    ...
    public virtual int DeleteItem(uint delItemOp, uint itemId)
    {
      ...
      if (node != null)
      {
        ...
        if (!String.IsNullOrEmpty(document))
        {
          ...
          string[] filesToBeDeleted = new string[1] { document };
          VSQUERYREMOVEFILEFLAGS[] flags = this.GetQueryRemoveFileFlags(filesToBeDeleted);
          if (!this.ProjectMgr.Tracker.CanRemoveItems(filesToBeDeleted, flags))
          {
            return VSConstants.S_OK;
          }
        }
    ...
    internal bool AddFileToNodeFromProjectReference(string projectRef, HierarchyNode targetNode)
    {
      ...
      {
        throw new ArgumentException(SR.GetString(SR.ParameterCannotBeNullOrEmpty), "projectRef");
      }
