...
namespace Microsoft.VisualStudio.Package
{
	public static class Utilities
	{
		...
		public static bool ContainsInvalidFileNameChars(string name)
		{
			...
			Url uri = new Microsoft.VisualStudio.Shell.Url(name);
		...
		internal static MSBuild.Project ReinitializeMsBuildProject(MSBuild.Engine buildEngine, string fullProjectPath, MSBuild.Project exitingBuildProject)
		{
			...
			if (exitingBuildProject != null && !NativeMethods.IsSamePath(exitingBuildProject.FullFileName, fullProjectPath))
			{
    ...
    public static string GetFileNameInProperCase(string FilePath)
    {
      ...
      FilePath = FilePath.Substring(0, FilePath.IndexOf(Path.DirectorySeparatorChar, 0) + 1);
      ...
      for (int i = 0; i < dirs.Length - 1; ++i)
      {
        ...
        FileSystemInfo[] infos = di.GetFileSystemInfos(dirs[i + 1]);
        if (infos.Length > 0)
